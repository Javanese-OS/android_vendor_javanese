...
function eat()
{
    if [ "$OUT" ] ; then
        ...
        echo "Found device"
        if (adb shell getprop ro.javanese.device | grep -q "$JAVANESE_BUILD"); then
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.javanese.device | grep -q "$JAVANESE_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.javanese.device | grep -q "$JAVANESE_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
